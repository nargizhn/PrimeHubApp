...
import { uploadVendorAttachment } from "../utils/vendorFiles";
...
export default function AddVendor() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (res.ok) {
                // 1) JSON body'den id dene
                let created = null;
                let vendorId = null;
                try {
                    created = await res.json();
                    vendorId = created?.id ?? created?.vendorId ?? created?.data?.id ?? null;
                } catch {
                    // 2) Body yoksa Location header'dan id dene
                ...
                // 3) Dosya varsa Storage + Firestore
                if (formData.images) {
                    if (!vendorId) {
                        console.warn("No vendorId in response; skipping attachment upload.");
                        alert("Vendor added, but API did not return vendor id. Attachment skipped.");
                    } else {
                        try {
                            await uploadVendorAttachment(String(vendorId), formData.images);
                        } catch (e) {
                            console.error("Attachment upload failed:", e);
                            alert(`Vendor added, but attachment could not be saved.`);
                        }
                    }
                }
            ...
            <div style={{ backgroundColor: "#fff", padding: 30, borderRadius: 12, boxShadow: "0 4px 12px rgba(0,0,0,0.1)", flexGrow: 1 }}>
                <form onSubmit={handleSubmit} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", columnGap: 50, rowGap: 20 }}>
                    ...
                    <div style={{ gridColumn: "span 2", display: "flex", flexDirection: "column", gap: 10, width: "100%" }}>
                        <label style={{ fontWeight: 600 }}>Upload Images / Files</label>
